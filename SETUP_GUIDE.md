...
### Step 3 — Update `main.dart` to Use FirebaseOptions

Open `lib/main.dart` and change the Firebase init line:
...
> ⚠️ The current code calls `Firebase.initializeApp()` with **no parameters**. This works ONLY if `google-services.json` (Android) or `GoogleService-Info.plist` (iOS) are correctly placed. Using `firebase_options.dart` is the recommended approach as a **fallback-safe** method.
...
- [ ] `lib/firebase_options.dart` exists (from FlutterFire CLI)
- [ ] `main.dart` uses `DefaultFirebaseOptions.currentPlatform`
